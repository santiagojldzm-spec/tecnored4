...
/* ══════════════════════════════════════
   ...
   Puntos que se conectan entre sí con líneas suaves.
   Se adapta al tema light/dark automáticamente.
══════════════════════════════════════ */
function initHeroCanvas() {
  ...
  class Dot {
    ...
    draw() {
      ...
      ctx.fillStyle = dark
        ? `rgba(10,132,255,${this.a})`
        : `rgba(10,100,255,${this.a * 0.7})`;
      ctx.fill();
    }
  }
  ...
  // Pausa cuando la pestaña no es visible (ahorro de CPU)
...
/* ══════════════════════════════════════
   GUÍAS RÁPIDAS — modal con contenido
══════════════════════════════════════ */
const GUIDES = {
  contrasena: {
    title: 'Cómo crear una contraseña segura',
    steps: [
      'Elige una frase que recuerdes fácilmente, como el nombre de tu mascota y el año en que nació.',
      'Agrega letras mayúsculas al inicio de cada palabra: <strong>MiPerro2008</strong>',
      'Añade al menos un símbolo al final: <strong>MiPerro2008!</strong> — esa es una contraseña segura.',
      'No uses tu nombre, fecha de nacimiento ni palabras como "password" o "123456".',
      'Guarda la contraseña escrita en un papel guardado en un lugar seguro de tu casa.',
      'Usa una contraseña diferente para cada cuenta importante.'
    ],
    tip: 'Una contraseña segura tiene: 8+ caracteres, mayúsculas, minúsculas, números y un símbolo.'
  },
  wifi: {
    title: 'Conectarse al WiFi paso a paso',
    steps: [
      'Desliza desde la parte superior de la pantalla hacia abajo para abrir el menú rápido.',
      'Toca el ícono de WiFi para activarlo si está apagado.',
      'Ve a <strong>Ajustes → WiFi</strong> y espera a que aparezca la lista de redes disponibles.',
      'Toca el nombre de tu red WiFi de la lista (usualmente el nombre de tu proveedor o uno personalizado).',
      'Escribe la contraseña del WiFi. Suele estar en una etiqueta pegada al router (la cajita con luces).',
      'Toca Conectar y espera unos segundos hasta que aparezca el ícono de WiFi en la barra superior.'
    ],
    tip: 'Si el WiFi no aparece en la lista, reinicia el router apagándolo 30 segundos y volviéndolo a encender.'
  },
  estafas: {
    title: 'Cómo reconocer mensajes falsos',
    steps: [
      '<strong>Premio sorpresa:</strong> si un mensaje dice que ganaste algo sin participar en nada, es una estafa. Bórralo.',
      '<strong>Urgencia extrema:</strong> mensajes que dicen "actúa AHORA o perderás tu cuenta" buscan que actúes sin pensar. Detente.',
      '<strong>Links sospechosos:</strong> antes de hacer clic, verifica que el enlace empiece con https:// y sea del sitio oficial.',
      '<strong>Piden datos personales:</strong> bancos y gobierno nunca piden contraseñas, PIN o número de tarjeta por mensaje.',
      '<strong>Errores de escritura:</strong> muchos mensajes falsos tienen faltas de ortografía o nombres mal escritos.',
      'Cuando tengas dudas, consulta con un familiar de confianza antes de hacer cualquier acción.'
    ],
    tip: 'Regla de oro: si algo parece demasiado bueno para ser verdad, o demasiado urgente, es una trampa.'
  },
  fotos: {
    title: 'Hacer copias de seguridad de tus fotos',
    steps: [
      'Descarga la app <strong>Google Fotos</strong> desde la Play Store (Android) o App Store (iPhone).',
      'Abre Google Fotos e inicia sesión con tu cuenta de Gmail.',
      'Toca tu foto de perfil en la esquina superior derecha y selecciona <em>Activar copia de seguridad</em>.',
      'Elige la opción <strong>"Solo con WiFi"</strong> para no gastar tus datos móviles.',
      'Deja el celular conectado al WiFi y enchufado. Las fotos se subirán automáticamente en segundo plano.',
      'Listo. Ahora, aunque pierdas el celular, tus fotos estarán seguras y las recuperas desde cualquier dispositivo.'
    ],
    tip: 'Google Fotos guarda tus fotos gratis en la nube. Si tienes muchas, puede tardar varias horas la primera vez.'
  },
  bateria: {
    title: 'Cómo hacer durar más la batería',
    steps: [
      'Ve a <strong>Ajustes → Pantalla → Brillo</strong> y bájalo al 40-50%. Esto es lo que más batería gasta.',
      'Activa el <strong>Modo de ahorro de energía</strong>: Ajustes → Batería → Modo de ahorro (Android) o Modo de bajo consumo (iPhone).',
      'Desactiva el Bluetooth y el WiFi cuando no los estés usando. Están en el menú deslizable superior.',
      'Reduce el tiempo de bloqueo automático: Ajustes → Pantalla → Tiempo de espera → ponlo en 30 segundos.',
      'Desactiva la actualización de apps en segundo plano: Ajustes → Aplicaciones → selecciona una → desactiva "Actualizar en segundo plano".',
      'Para cargar correctamente: carga cuando llegue al 20% y desconecta al llegar al 100%. Evita dejarlo cargando toda la noche.'
    ],
    tip: 'Con estos ajustes puedes extender la duración de tu batería entre 1 y 2 horas adicionales por carga.'
  },
  lento: {
    title: 'Cómo acelerar un celular lento',
    steps: [
      'Cierra todas las apps abiertas: toca el botón de apps recientes (cuadrado en Android) y ciérralas todas deslizando.',
      '<strong>Reinicia el celular</strong>. Parece simple pero limpia la memoria temporal y soluciona la mayoría de lentitudes.',
      'Libera espacio: Ajustes → Almacenamiento → elimina apps que no uses y fotos duplicadas o antiguas.',
      'Limpia el caché de apps: Ajustes → Aplicaciones → selecciona la app más usada → Almacenamiento → Limpiar caché.',
      'Desinstala apps que no uses. Cada app ocupa espacio y algunos funcionan en segundo plano consumiendo recursos.',
      'Actualiza el sistema: Ajustes → Acerca del teléfono → Actualización de software. Las actualizaciones mejoran el rendimiento.'
    ],
    tip: 'Si el celular tiene más de 4-5 años y sigue lento con estos pasos, puede ser momento de considerar un reemplazo.'
  }
};
...
function openGuide(key) {
  ...
  body.innerHTML = `
    <h2 class="guide-modal-title" id="guide-modal-title">${guide.title}</h2>
    ...
    ${guide.tip ? `<div class="guide-tip">${guide.tip}</div>` : ''}`;
...
// Listeners para guías
document.addEventListener('DOMContentLoaded', function() {
  // Abrir guía al hacer click en botones de guía
  document.addEventListener('click', function(e) {
    const btn = e.target.closest('[data-action="guide"]');
    if (btn) { e.stopPropagation(); openGuide(btn.dataset.guide); }
  });

  // Cerrar modal de guía
  const closeBtn = document.getElementById('guide-modal-close');
  const scrim    = document.getElementById('guide-modal-scrim');
  if (closeBtn) closeBtn.addEventListener('click', closeGuideModal);
  if (scrim)    scrim.addEventListener('click', closeGuideModal);

  // Inicializar hero canvas
  initHeroCanvas();
});
